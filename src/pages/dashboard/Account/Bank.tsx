...
const Bank = () => {
  const { data: userBanks, isLoading, error } = useQuery(['userBanks'], fetchBanks);
    ...
    <div>
...

      {
        isLoading ?
          ...
          :
          error ?
            <p>Error while fetching user banks</p>
            :
            (
              userBanks?.data?.length > 0 ?
                <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-8'>
                  {
                    userBanks.data.map((bank: BankCardType) => (
                      <BankCard
                        key={bank.id}
                        id={bank.id}
